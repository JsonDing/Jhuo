...
public class SelfIssueGoodsAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public void onBindViewHolder(final RecyclerView.ViewHolder holder, final int position) {
      if(holder instanceof BodyViewHolder) {
            GlideUtils.glidNormleFast(mActivity,((BodyViewHolder) holder).imgsGoods,ConUtils.SElF_GOODS_IMAGE_URL +
                    listBean.get(position-1).getPic().split(",")[0]);
            ((BodyViewHolder) holder).tvGoodsName.setText(listBean.get(position-1).getName());
            ((BodyViewHolder) holder).tvTime.setText(DateTimeUtils.getTime(
            ...
            ((BodyViewHolder) holder).itemView.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    if(null!=onClickListener){
                        onClickListener.lookDetials(position-1,listBean.get(position-1));
                    }
                }
            });
            ((BodyViewHolder) holder).btnRemindMe.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    if(null!=onClickListener){
                        onClickListener.remindMeClick(String.valueOf(listBean.get(position-1).getNumber()));
                    }
                }
            });
        }else{
    ...
    public interface OnClickListener{
        void lookDetials(int position,GetSelfGoodsResultBean.SuccessBean.ListBean detialsBean);
